...
const sayHelloButtonChange  = (event) => {
...
sayHelloButton.addEventListener('mouseover', sayHelloButtonChange)
...
const sayHelloButtonChangeBack  = (event) => {
    event.preventDefault()
...
sayHelloButton.addEventListener('mouseout', sayHelloButtonChangeBack)
...
const basURL = 'http://localhost:3000/animals'
    ...
    axios.get(`${basURL}`)
        .then(response => {
            ...
            for(let i = 0; i < response.data.length; i++){
                ...
                // newThing.textContent = response[i]
                // newThing.append('body')
                document.querySelector('body').innerHTML += `<p>${response.data[i]}</p>`
            }
        })
...
const repeatMyParam = (str) => {
    ...
    axios.get(`http://localhost:3000/repeat/${str.target.textContent}`)
        .then(response => {
            console.log(response.data);
...
const queryGet = (event) => {
    event.preventDefault()

    axios.get(`http://localhost:3000/query-test/?query1=Pokemon&query2=Digimon`)
        .then(response => {
            console.log(response.data);
        })
    .catch(error => console.log('error'))
...
document.getElementById('query-button').addEventListener('click', queryGet)
...
gave an error but also logged 'You sent an empty query' in the console.
    2: Send more than 1 query on the URL -- what happened? 
    sent both queries and logged 'You sent more than 1 query!'
*/
...
const createFood = (event) => {
    ...
    foodText.textContent = foodInput.value
    ...
    .then(res => {
        ...
        for(let i = 0; i < res.data.length; i++){
            document.querySelector('ul').innerHTML //+= `<p>${res.data[i]}</p>`
        }
    })
    .catch(error => console.log('error'))
